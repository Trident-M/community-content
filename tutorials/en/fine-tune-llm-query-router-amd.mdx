---
title: "Fine-Tune a Query Router to Cut LLM Costs for AI Hackathons"
description: "Fine-tune a query router for AI hackathons that cuts LLM costs by routing queries to cheaper Fireworks-hosted models, without losing accuracy on hard queries."
image: "https://imagedelivery.net/K11gkZF3xaVyYzFESMdWIQ/fine-tune-llm-query-router-amd-cover/public"
authorUsername: "stevekimoi"
---

Watch the router make its decisions live before diving in:

<LiteYouTube videoId="lmtmKMZ1AOs" containerClassName={"youtubeContainer"} />
...
> **Disclaimer:** if you're participating in the [AMD Developer Hackathon: ACT II](https://lablab.ai/ai-hackathons/amd-developer-hackathon-act-ii), the official Participant FAQ lists the **MiniMax** and **Kimi K** series as the models you may use on Fireworks AI. The two model IDs above are stand-ins from the serverless catalog at the time of writing, used to demonstrate the methodology, so swap in the IDs from the hackathon's `ALLOWED_MODELS` list before submitting. That's exactly why they live in environment variables.
...
![Streamlit demo UI immediately after starting, sidebar showing model tiers and zeroed session totals](https://imagedelivery.net/K11gkZF3xaVyYzFESMdWIQ/demo-startup/public)
...
![Demo UI after running one query, showing a bar chart comparing fine-tuned router tokens against prompt-based baseline tokens](https://imagedelivery.net/K11gkZF3xaVyYzFESMdWIQ/demo-chart-after-query/public)
...
![Demo UI with a tooltip showing exact token values on hover, plus a query log table below the chart](https://imagedelivery.net/K11gkZF3xaVyYzFESMdWIQ/demo-tooltip-and-log/public)
...
![Sidebar detail showing session totals after one query: 119 fine-tuned tokens vs 314 baseline tokens, 195 tokens saved, 62% fewer tokens](https://imagedelivery.net/K11gkZF3xaVyYzFESMdWIQ/demo-sidebar-totals/public)
...
Yes. Any hackathon that scores token efficiency, including LLM hackathons and other agentic AI hackathons, rewards routing to the cheapest model that can still pass the accuracy bar. The methodology here (empirically label your own data, fine-tune small, evaluate against a real baseline) transfers directly.
